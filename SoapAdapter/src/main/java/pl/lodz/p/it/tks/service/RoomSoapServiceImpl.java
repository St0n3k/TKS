...
public class RoomSoapServiceImpl implements RoomSoapService {
    ...
    @Override
    public ApartmentSoapDTO addApartment(@Valid @WebParam(name = "dto", mode = WebParam.Mode.IN) CreateApartmentSoapDTO dto)
            throws CreateRoomException {
    ...
    public RoomSoapDTO updateRoom(@WebParam(name = "id", mode = WebParam.Mode.IN) UUID id,
                                  @Valid @WebParam(name = "dto", mode = WebParam.Mode.IN) UpdateRoomSoapDTO dto) throws RoomNotFoundException, UpdateRoomException {
        Room room = roomCommandUseCase.updateRoom(id, new Room(dto.getRoomNumber(), dto.getPrice(), dto.getSize()));
        return roomSoapMapper.mapToDto(room);
    }

    @Override
    public ApartmentSoapDTO updateApartment(@WebParam(name = "id", mode = WebParam.Mode.IN) UUID id,
                                            @Valid @WebParam(name = "dto", mode = WebParam.Mode.IN) UpdateApartmentSoapDTO dto) throws InvalidInputException, RoomNotFoundException, UpdateRoomException {
